 ...
 * 
 * This is a utility wrapper so that metadata for any number of columns can be
 * cached in a collection and accessed repeatedly and in any order. Column
 * metadata are typically returned from the database as a forward-only result
 * set, and cannot be "rewound" after scrolling forward to a specific column.
 * 
 * @author Joe Kearns
 * @version $Revision$
 * @since June 2017
...
public class ColumnMetaData
{
    ...
    private final StringMetaDataField tableCat =
            new StringMetaDataField("tableCat", 1);
    private final StringMetaDataField tableSchema =
            new StringMetaDataField("tableSchema", 2);
    private final StringMetaDataField tableName =
            new StringMetaDataField("tableName", 3);
    private final StringMetaDataField columnName =
            new StringMetaDataField("columnName", 4);
    private final IntegerMetaDataField sqlType =
            new IntegerMetaDataField("sqlType", 5);
    private final StringMetaDataField sqlTypeName =
            new StringMetaDataField("sqlTypeName", 6);
    private final IntegerMetaDataField columnSize =
            new IntegerMetaDataField("columnSize", 7);
    // 8 not used
    private final IntegerMetaDataField decimalDigits =
            new IntegerMetaDataField("decimalDigits", 9);
    private final IntegerMetaDataField numPrecRadix =
            new IntegerMetaDataField("numPrecRadix", 10);
    private final IntegerMetaDataField nullable =
            new IntegerMetaDataField("nullable", 11);
    private final StringMetaDataField remarks =
            new StringMetaDataField("remarks", 12);
    private final StringMetaDataField columnDefault =
            new StringMetaDataField("columnDefault", 13);
    // 14 not used
    // 15 not used
    private final IntegerMetaDataField charOctetLength =
            new IntegerMetaDataField("charOctetLength", 16);
    private final IntegerMetaDataField ordinalPosition =
            new IntegerMetaDataField("ordinalPosition", 17);
    private final StringMetaDataField isoNullable =
            new StringMetaDataField("isoNullable", 18);
    private final StringMetaDataField scopeCatalog =
            new StringMetaDataField("scopeCatalog", 19);
    private final StringMetaDataField scopeSchema =
            new StringMetaDataField("scopeSchema", 20);
    private final StringMetaDataField scopeTable =
            new StringMetaDataField("scopeTable", 21);
    private final ShortMetaDataField sourceDataType =
            new ShortMetaDataField("sourceDataType", 22);
    private final StringMetaDataField autoincrement =
            new StringMetaDataField("autoincrement", 23);
    private final StringMetaDataField generatedcolumn =
            new StringMetaDataField("generatedcolumn", 24);
     ...
     *
     * @param resultSet
     *            column metadata results from DatabaseMetaData.getColumns
     * @throws SQLException
     *             thrown on from ResultSet accessors
     */
    ...
    /**
     * Use a DataType factory to create a DataType instance of the type
     * appropriate to the metadata.
     * 
     * TODO It would be good to have an equivalent factory method defined in
     * IDataTypeFactory but that would impact third-party implementations of the
     * interface (breaking change), so could only be done as part of a major
     * version release.
     * 
     ...
     */
    public DataType getDataType(IDataTypeFactory dataTypeFactory)
            throws DataTypeException
    {
        return dataTypeFactory.createDataType(this.getSQLType(),
                this.getSqlTypeName(), this.getTableName(),
                this.getColumnName());
    }
    ...
    public int hashCode()
    {
        ...
        result = prime * result
                + ((generatedcolumn == null) ? 0 : generatedcolumn.hashCode());
        result = prime * result
                + ((isoNullable == null) ? 0 : isoNullable.hashCode());
        result = prime * result
        ...
        result = prime * result
                + ((scopeCatalog == null) ? 0 : scopeCatalog.hashCode());
        result = prime * result
    ...
    private abstract class MetaDataField
    {
        ...
        protected MetaDataField(String name, int index)
        {
            ...
            this.name = name;
            this.index = index;
            // Every instance created must be added to the parent object's list
            // of fields
        ...
        // Having verified that a value for this field is present in the result
        // set, assign it
    ...
    private class StringMetaDataField extends MetaDataField
    {
        ...
        {
            return "StringMetaDataField [name=" + name + ", index=" + index
                    + ", value=" + value + "]";
        }
    ...
    private class IntegerMetaDataField extends MetaDataField
    {
        ...
        public String toString()
        {
            return "IntegerMetaDataField [name=" + name + ", index=" + index
                    + ", value=" + value + "]";
        }
    ...
    private class ShortMetaDataField extends MetaDataField
    {
        ...
        public String toString()
        {
            return "ShortMetaDataField [name=" + name + ", index=" + index
                    + ", value=" + value + "]";
        }
