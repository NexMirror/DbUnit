...
public class SQLHelperTest extends AbstractHSQLTestCase
{
    ...
    public void testGetDatabaseInfoWithException() throws Exception
    {
        ...
        assertTrue(info
                .indexOf(SQLHelper.ExceptionWrapper.NOT_AVAILABLE_TEXT) > -1);
    }

    /**
     * Regression test for changes to SQLHelper.createColumn. Also covers
     * org.dbunit.dataset.ColumnMetaData
     */
    public void testCreateColumn()
    {
        try
        {
            _testCreateColumn(new Object[] {"CAT", "SCHEMA", "table_1", "col01",
                    12, "VARCHAR", 30, null, 2, 2, 0, "Test data", null, null,
                    null, 30, 4, "YES", null, null, null, null, "NO", "NO"});
            _testCreateColumn(new Object[] {"CAT", "SCHEMA", "other_table",
                    "some_id", 3, "DECIMAL", 10, null, 2, 2, 0, "Primary key",
                    "0.00", null, null, 4, 1, "NO", null, null, null, null,
                    "YES", "YES"});
            _testCreateColumn(new Object[] {"CAT", "SCHEMA", "same_table",
                    "some_ud_ref", -3, "VARBINARY", 8, null, 0, 0, 1,
                    "User defined ref", null, null, null, null, 7, "YES",
                    "Scope cat", "Scope schema", "Scope table", (short) 12,
                    "YES", "YES"});
        } catch (Exception e)
        {
            e.printStackTrace();
    ...
    private void _testCreateColumn(ResultSet resultSet,
            IDataTypeFactory dtFactory) throws Exception
    {
        Column oldCol = oldCreateColumn(resultSet, dtFactory, true);
        ColumnMetaData colmd = new ColumnMetaData(resultSet);
        // Check that the new method creates a column equal to the on returned
        // by the legacy method:
        Column newCol = SQLHelper.createColumn(colmd, dtFactory, true);
        assertEquals(oldCol, newCol);
        // Check that the backwards-compatibility method also produces a
        // congruent column:
    ...
    /*
     * This is a direct lift of the previous SQLHelper.createColumn, for
     * comparison with the new implementation in regression testing.
     */
            ...
            throws SQLException, DataTypeException
    {
        ...
        // This is only available since Java 5 - so we can try it and if it does
        // not work default it
        ...
        {
            Column column = new Column(columnName, dataType, sqlTypeName,
                    Column.nullableValue(nullable), columnDefaultValue, remarks,
                    Column.AutoIncrement.autoIncrementValue(isAutoIncrement));
            return column;
        } else
        {
            if (datatypeWarning)
                System.err.println(tableName + "." + columnName + " data type ("
                        + sqlType + ", '" + sqlTypeName
